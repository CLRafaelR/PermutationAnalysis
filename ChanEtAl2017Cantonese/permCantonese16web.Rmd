...
3. Plot the mean proportion of gaze to `target` using `ggplot2::ggplot()`
...
5. `Join` that filtered data to the original `bothdata` by the columns `Participant`, `Item`, `HeadNoun`, and `SentenceType`; at the same time, attach `suffix`es to the duplicate columns (especially the column `target`), so that you can identify which data frame the duplicate columns originally came from.
6. `Mutate` the joined data, so that its `target` indicates the subtraction of `target.filtered` from `target.original`
